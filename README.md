...
npm run i18n:check          # locale completeness + placeholder parity across all locales
```
...
**Codes from another shop.** For a giveaway run together with another creator, codes can be entered by hand (`couponManualCode`, `couponManualCodesPerPrize`, `couponManualNote`): one for everybody, or one per prize slot. They need **no Tebex store of your own**, and they are independent of a generated coupon: configure both and the winner receives both codes in the same DM, one per shop. The bot only delivers an entered code, so it can neither validate it nor revoke it on a reroll — the latter is logged to the guild's log channel rather than passed over silently.
